...
This application builds an independent version of Sonarr on your NAS which can be used specifically for kids shows to make it easier to segregate

## Usage

Set `kids_sonarr_enabled: true` in your `/inventories/[my inventory]/group_vars/nas.yml` file.

The Sonarr web interface can be found at `http://ansible_nas_host_or_ip:8990` by default

## Specific Configuration

**First make sure Sonarr has permissions to write and read the `/download` and `/tv` folders**. Do this by ensuring the `kids_sonarr_movies_directory:` and `kids_sonarr_download_directory` settings are correct.
